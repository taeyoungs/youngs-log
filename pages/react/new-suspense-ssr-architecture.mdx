...
<Callout type="info">번역 글입니다. - [Original Post](https://github.com/reactwg/react-18/discussions/37)</Callout>
...
![페이지에서 사용자와 상호 작용 가능한 영역](/react/New-Suspense-SSR-Architecture-in-React-18/1.png)
...
![빈 페이지 예시](/react/New-Suspense-SSR-Architecture-in-React-18/2.png)
...
![아직 완전히 사용자와 상호 작용 가능하지 않은 스크린 영역](/react/New-Suspense-SSR-Architecture-in-React-18/3.png)
...
![완전히 사용자와 상호 작용 가능한 스크린 영역](/react/New-Suspense-SSR-Architecture-in-React-18/4.png)
...
![Spinner를 포함한 HTML Streaming](/react/New-Suspense-SSR-Architecture-in-React-18/5.png)
...
![hydrate된 부분과 hydrate 되지 않은 부분이 모두 존재하는 HTML](/react/New-Suspense-SSR-Architecture-in-React-18/3.png)
...
![Spinner를 포함하며 hydrate된 HTML Streaming](/react/New-Suspense-SSR-Architecture-in-React-18/7.png)

`Comments`에 대한 **HTML** 로딩이 끝났을 때, 이는 **JavaScript**가 아직 준비가 되지 않았기 때문에 비대화형(상호 작용이 불가능한 상태)으로 보여질 것이다.

![hydrate된 부분과 데이터의 로딩이 끝났지만 hydrate 되지 않은 부분이 존재하는 HTML](/react/New-Suspense-SSR-Architecture-in-React-18/8.png)
...
![나머지 부분을 hydrating 하고 있을 때 Sidebar를 클릭한 경우](/react/New-Suspense-SSR-Architecture-in-React-18/9.png)
...
![Sidebar와 Comments의 HTML은 로딩됐으나 JavaScript 코드는 로딩되지 않은 상태](/react/New-Suspense-SSR-Architecture-in-React-18/10.png)
...
![Sidebar의 hydrating이 진행되고 있는 상태](/react/New-Suspense-SSR-Architecture-in-React-18/11.png)

하지만 이때 사용자가 댓글 위젯과 상호 작용하려고 한다고 해보자.

![Sidebar의 hydrating이 진행되고 있는 상태에서 Comments를 사용자가 클릭한 경우](/react/New-Suspense-SSR-Architecture-in-React-18/12.png)

**React**는 클릭 이벤트의 캡처링 단계 동안 댓글 영역을 동시에 `hydrate` 할 것이다.

![Comments의 hydrating이 진행되고 있는 상태](/react/New-Suspense-SSR-Architecture-in-React-18/13.png)

결과적으로, 댓글 영역은 클릭을 처리할 수 있고 사용자와의 상호 작용에 반응할 수 있도록 적절하게 `hydrate` 된다. 그리고 나서, **React**는 더 이상 급할 게 없으므로 **Sidebar**를 `hydrate` 한다.

![Comments의 hydrating을 끝내고 Sidebar의 hydrating이 진행되고 있는 상태](/react/New-Suspense-SSR-Architecture-in-React-18/14.png)
...
![Sidebar의 hydrating이 진행되고 있는 상태에서 Comments를 사용자가 클릭한 경우](/react/New-Suspense-SSR-Architecture-in-React-18/15.png)
