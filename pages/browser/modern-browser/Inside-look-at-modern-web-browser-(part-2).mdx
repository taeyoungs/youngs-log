...
<Callout type="info">번역 글입니다. - [Original Post](https://developer.chrome.com/blog/inside-browser-part2/)</Callout>
...
![Figure 1: Browser UI at the top, diagram of the browser process with UI, network, and storage thread inside at the bottom](/browser/modern-browser/part2/1.png)
...
![Figure 1: UI Thread asking if the input is a search query or a URL](/browser/modern-browser/part2/2.png)
...
![Figure 3: the UI thread talking to the network thread to navigate to mysite.com](/browser/modern-browser/part2/3.png)
...
![Figure 4: response header which contains Content-Type and payload which is the actual data](/browser/modern-browser/part2/4.png)
...
![Figure 5: Network thread asking if response data is HTML from a safe site](/browser/modern-browser/part2/5.png)
...
![Figure 6: Network thread telling UI thread to find Renderer Process](/browser/modern-browser/part2/6.png)
...
![Figure 7: IPC between the browser and the renderer processes, requesting to render the page](/browser/modern-browser/part2/7.png)
...
![Figure 8: IPC from the renderer to the browser process to notify the page has 'loaded'](/browser/modern-browser/part2/8.png)
...
![Figure 9: IPC from the browser process to a renderer process telling it that it's about to navigate to a different site](/browser/modern-browser/part2/9.png)
...
![Figure 10: 2 IPCs from a browser process to a new renderer process telling to render the page and telling old renderer process to unload](/browser/modern-browser/part2/10.png)
...
![Figure 11: the network thread in the browser process looking up service worker scope](/browser/modern-browser/part2/11.png)
...
![Figure 12: the UI thread in a browser process starting up a renderer process to handle service workers; a worker thread in a renderer process then requests data from the network](/browser/modern-browser/part2/12.png)
...
![Figure 13: the UI thread in a browser process starting up a renderer process to handle service worker while kicking off network request in parallel](/browser/modern-browser/part2/13.png)
